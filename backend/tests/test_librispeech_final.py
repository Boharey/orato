...
Captures filler words, stores rich JSON for analysis, supports batching.
Run: uv run python tests/test_librispeech_enhanced.py --max 100
"""
...
def test_librispeech_enhanced(max_samples=100, batch_id=1, output_dir=None):
    ...
    print(f"Streaming LibriSpeech test-clean, processing up to {max_samples} samples...")
    ...
    all_filler_rates = []
    all_filler_counts = []  # list of total_fillers per sample
    filler_words_global = {}  # aggregate filler word counts across samples
    ...
    for idx, sample in enumerate(dataset):
        ...
        if not out or out.get("word_count", 0) == 0:
            print(f"[{idx+1}] Skipped (engine empty)")
            continue
        ...
        count += 1
        print(f"[{count}/{max_samples}] WER:{wer:.4f} | Actual:{actual_score:.1f} | Ref:{ref_score:.1f} | Fillers:{filler_rate:.1f}%")
    ...
    out_file = output_dir / f"librispeech_batch_{batch_id}_{count}_samples.json"
    with open(out_file, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    
    print("\n" + "="*60)
    print(f"BATCH {batch_id} SUMMARY on {count} samples")
    print("="*60)
...
if __name__ == "__main__":
    ...
    test_librispeech_enhanced(args.max, args.batch, args.output_dir)